...
import com.inspireon.dragonfly.model.domain.image.Image;
...
public class ImageUtils {	
	...
	public static Size createAndResize(String sourcePath, String fileName, Size size, boolean cropped) throws IOException {
		...
		if (cropped || sourceImg.getHeight() > sourceImg.getWidth()) sourceImg = cropImage(sourceImg);				
		
		Size newSize = resize(new Size(sourceImg.getWidth(), sourceImg.getHeight()), size);
		destImg = Scalr.resize(sourceImg, Scalr.Method.ULTRA_QUALITY, newSize.width(), newSize.height()); 
...
//    	createAndResize("test.jpg", "test1", ImageProperty.FEATURED_LARGE, false);
//    	createAndResize("test.jpg", "test2", ImageProperty.FEATURED_SMALL, false);  	
    }
}
